...
const checks = [
  {
    ...
    test: (s) => /((^|\s)\.\.\.(\s|$))/.test(s),
    fix: "استبدال المقاطع المبتورة بنسخ كاملة بدون ...",
  },
  {
    id: "audience-old",
    label: "Audience خاطئ (api.athar)",
    test: (s) =>
      /getToken\s*\(\s*\{\s*audience:\s*["']https?:\/\/api\.athar["']\s*\}\s*\)/.test(
        s
      ),
  ...
  },
  {
    ...
    test: (s) =>
      /getToken\s*\(\s*\{\s*audience:\s*["']https?:\/\/api\.n-athar["']\s*\}\s*\)/.test(
        s
      ) && !/getTokenSilently/.test(s),
  ...
  },
  {
    ...
    test: (s, p) =>
      p.includes(path.join("netlify", "functions")) &&
      /Access-Control-Allow-Origin['"]?\s*:\s*['"]\*/.test(s),
  ...
  },
  {
    ...
    test: (s, p) =>
      p.includes(path.join("netlify", "functions")) &&
      !/require\(["']\.\/_cors(\.js)?["']\)/.test(s) &&
      /exports\.handler/.test(s),
  ...
  },
  {
    ...
    test: (s, p) =>
      p.endsWith(path.join("netlify", "functions", "remind-miyad.js")) &&
  ...
  },
  {
    ...
    test: (s, p) =>
      (p.endsWith(".html") || p.includes(path.join("assets", "js"))) &&
      !/admin-announcement\?latest=1/.test(s),
  ...
  },
  {
    ...
    test: (s, p) =>
      ...
      /invoices-list/.test(s) &&
      !/from\("invoices"\)/.test(s),
  ...
  },
  {
    ...
    test: (s, p) =>
      p.includes(path.join("assets", "js", "require-auth.js")) &&
      !/trial/.test(s),
...
function analyzeFile(rel) {
  ...
  try {
    ...
    return { file: rel, ok: false, errors: ["قراءة الملف فشلت"] };
  }
  const hits = [];
  for (const c of checks) {
    try {
      if (c.test(content, rel))
        hits.push({ id: c.id, label: c.label, fix: c.fix });
    } catch {}
  }
...
function main() {
  ...
  console.log(
    "إن أردتِ JSON كامل: أضيفي > audit.json عند التشغيل وافتحيه بجدول."
  );
