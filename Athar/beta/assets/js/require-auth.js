...
(function AtharGuard() {
  ...
  const DEBUG = false;
  const log = (...a) => {
    if (DEBUG) console.info("[AtharGuard]", ...a);
  };
  const warn = (...a) => {
    if (DEBUG) console.warn("[AtharGuard]", ...a);
  };
  const err = (...a) => {
    console.error("[AtharGuard]", ...a);
  };
  ...
  const toPricing = (msg) => {
    try {
      if (msg) sessionStorage.setItem("athar:msg", msg);
    } catch {}
  ...
  async function cleanupRedirectIfNeeded(client) {
    if (
      ...
      const desired =
        ...
        (function () {
          try {
            return localStorage.getItem("afterLogin") || null;
          } catch (_) {
            return null;
          }
        })();
      ...
      if (
        desired &&
        desired !== location.pathname + location.search + location.hash
      ) {
        try {
  ...
  async function fetchUserStatus(client) {
    try {
      ...
      const res = await fetch(
        "/.netlify/functions/user-status?ts=" + Date.now(),
        {
  ...
  async function enforce() {
    try {
      ...
      let status = await fetchUserStatus(client).catch(() => ({
        active: false,
        status: "inactive",
      }));
      ...
      if (slug === ADMIN) {
        try {
          ...
          const isAdmin =
            ...
            (Array.isArray(claims?.[NS + "roles"]) &&
              claims[NS + "roles"].includes("admin")) ||
            (Array.isArray(claims?.[ALT + "roles"]) &&
              claims[ALT + "roles"].includes("admin"));
      ...
      if (TOOLS.has(slug)) {
        if (!status.active) {
          return toPricing(
            "حسابك غير مُفعّل (انتهت التجربة أو لم يتم التفعيل)."
          );
        }
        unmountGuardOverlay();
        fireAuthReady();
      ...
      if (!status.active) {
        const msg =
          status.status === "expired" || status.status === "inactive"
