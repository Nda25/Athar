...
// إرجاع فواتير المستخدم الحالي (محمي بـ Auth0)
...
exports.handler = async (event) => {
  ...
  try {
    // نقرأ أحدث 50 حركة دفع تخص المستخدم
    const { data, error } = await supabase
      .from("payments_log")
      .select("created_at,gateway,event_type,object,status,amount,currency,provider_event_id,invoice_id,email,user_sub,amount_sar")
